...
export function useCarrosselData(csvUrl: string) {
  const [cards, setCarrossel] = useState<CardData[]>([]);
  ...
  useEffect(() => {
    const fetchCarrossel = async () => {
      try {
        ...
        setCarrossel(formatted);
      } catch (err) {
        console.error("Erro ao buscar ou processar CSV:", err);
        notification.error({
          message: "Erro",
          description: "Não foi possível carregar os Carrossel via CSV.",
        });

...
const CardCombosLoading: React.FC<CardCombosProps & { isLoading: boolean }> = ({
    card,
    activeIndex,
    setActiveIndex,
    index,
        ...
        >
            ...
            >
                <div className="relative overflow-hidden flex flex-col items-center rounded-[20px] border border-black h-full">
                    ...
                    >
                        <h3 className={`text-5xl text-center uppercase font-[700] ${isLoading ? "blur-sm" : ""}`}>
                            {words.slice(0, middleIndex).join(" ")}
                        </h3>
                        <h3
                            className={`text-5xl text-center uppercase font-[700] border-b-2 border-senary w-[300px] mb-2 ${isLoading ? "blur-sm" : ""}`}
                        >
                            {words.slice(middleIndex).join(" ")}
                        </h3>

                        <p className={`text-sm text-center ${isLoading ? "blur-sm" : ""}`}>
                            {card.description}
                        </p>
                        <Button
                            type="primary"
...
export default React.memo(CardCombosLoading);